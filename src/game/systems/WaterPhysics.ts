...
import { CHUNK_TILES, WORLD_CHUNKS, isWaterTileFast, tileToIso, isoToTile, ISO_TILE_W, ISO_TILE_H } from "../world/Terrain";

/**
 * WaterPhysics.ts - Sistema de física para agua.
 * Crea cuerpos estáticos Arcade para cada tile de agua en chunks visibles (3x3 alrededor de cámara)
 * y los sincroniza con el movimiento de cámara. El jugador y NPCs colisionan y no pueden atravesarla,
 * tampoco pueden spawnear sobre ella (ver SpawnSystem + Terrain.isBlockedTile).
 */
export class WaterPhysicsManager {
  ...
  private ensureTexture() {
    if (!this.scene.textures.exists("water_pixel")) {
      const g = this.scene.add.graphics();
      g.fillStyle(0xffffff, 1);
      g.fillRect(0, 0, 2, 2);
      g.generateTexture("water_pixel", 2, 2);
      g.destroy();
    }
    // Fallback reutiliza mineral_pixel si existe para no duplicar
  ...
  /** Sincroniza colliders de agua para los 3x3 chunks alrededor de la cámara. Throttle 512px. */
  ...
  private createChunk(cx: number, cy: number): void {
    ...
    for (let y = 0; y < CHUNK_TILES; y++) {
      for (let x = 0; x < CHUNK_TILES; x++) {
        ...
        if (!isWaterTileFast(wx, wy)) continue;
        const iso = tileToIso(wx, wy);
        const isoX = iso.x + ISO_TILE_W/2;
        const isoY = iso.y + ISO_TILE_H/2;
        const img = this.group.create(isoX, isoY, "water_pixel") as Phaser.Physics.Arcade.Image;
...
export default WaterPhysicsManager;
