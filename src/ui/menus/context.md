...
| `Console.tsx:7` | 157 | Consola dual `chat|console` vía `hooks/menu/useConsole`. `open false→return null` (sin hint flotante). Si `open` render `absolute bottom:0 0a0af2 backdrop blur 4` toggle `Chat|Consola` (verde/blue active), input `mono 12px #1a1a1a` placeholder `menu | spawnGhost2 | ...` + burbuja chat, `execute`/`closeConsole` handlers, feedback+history `maxHeight 60` 8 líneas. Comandos create/spawn/godmode/fullmode mutan vía API validada (JWT). Renderiza `CommandMenuPanel` cuando `menuOpen`. **Bloquea** vía `setConsoleOpen` → `isGameInputBlocked`. |
| `CommandMenuPanel.tsx` | — | Panel modal del comando `menu`: 4 categorías (Npc/Mobs/Items/Dev) desde `hooks/menu/consoleCommands`. Click escribe el comando en la consola (ENTER ejecuta); entradas con cantidad (▸) despliegan selector con su rango y el número elegido completa el comando. Badge `GodMode ON/OFF` (estado del servidor). Cierra con ✕, click fuera o ESC. |
...
- `hooks/menu/useConsole:137` → `open,mode,switchMode,input,setInput,history,feedback,inputRef,execute,closeConsole,menuOpen,openMenu,closeMenu,godMode` (regex `createnpc1..10`→spawn-allow→`phaser-create-npcs`, `spawnGhost1..3`, `menu`→panel, chat→`phaser-chat-bubble`, phaser-npcs-spawned history)
