...
## Propósito
**Fuente de verdad** del input remapable. Define 15 acciones, defaults, helpers, buffers y flags de bloqueo. Adaptado por `game/systems/InputSystem` para Phaser.
...
**`GameAction:10` 15 acciones:** `move_up/down/left/right, jump, dash, attack, attackAlt, inventory, map, missions, stats, tutorial, close, cameraFollow (Y)`
...
- `setInventoryOpen` (PlayerInventoryPanel), `setRebinding/isRebindingActive` (KeybindsEditor), `setConsoleOpen/isConsoleOpenActive` (Console + `document.activeElement INPUT/TEXTAREA/contentEditable`), `isGameInputBlocked()` true si `rebinding||consoleOpen||inventoryOpen||activeElement input||window.__lordsConsoleOpen`
...
## Para Repomix
Nueva `GameAction` → añadir en `GameAction` union + `BINDING_INFOS` + wrapper `InputSystem.isXJustPressed`. Respetar `isGameInputBlocked()` en todo input gameplay.
